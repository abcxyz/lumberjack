...
type Validator interface {
	...
	ValidateJWT(ctx context.Context, jwtStr, expectedSubject string) (jwt.Token, error)
}
...
func (p *Processor) Process(ctx context.Context, logReq *api.AuditLogRequest) error {
	...
	token, err := p.validator.ValidateJWT(ctx, jvsToken, principalEmail(logReq))
	if err != nil {
		return fmt.Errorf("%w: %w", auditerrors.ErrJustificationInvalid, err)
	}
...
func principalEmail(logReq *api.AuditLogRequest) string {
	if logReq == nil {
		return ""
	}
	if logReq.Payload == nil {
