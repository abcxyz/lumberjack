...
JAVA_HELLO_BUILD_COMMAND=${ROOT}/clients/java-logger/scripts/build_server.sh
...
terraform -chdir=${TF_CI_WITH_SERVER_DIR} apply -auto-approve \
  ...
  -var="hello_build_command=${JAVA_HELLO_BUILD_COMMAND}" \
  -var=${ENV_VARS} \
...
mvn verify
java -cp server-test/target/server-test-0.0.1.jar abcxyx.helloworld.HelloWorldClientTls ${HELLO_ENDPOINT} 443 ${ID_TOKEN}